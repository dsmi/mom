%
% Rectangular cavity with two ports, to compare the NaivePdn results against.
%
...
l = 300.0*mil2meter;  % cavity length -- x size
w = 100.0*mil2meter;  % cavity width -- y size
...
% angular frequencies
%freqs = 1e9*2*pi;
freqs = linspace(1e7, 4e10, 400)*2*pi;
...
tswrite('rectcavity_mom.y2p', freqs/(2*pi), Y1, 'Y', 50);
